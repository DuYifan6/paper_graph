# -*- coding: utf-8 -*-
"""
Graph3 内嵌版 RealGraph
- 读取 common_preprocess_fixedsplit.py 生成的公共缓存
- 不再依赖外部预生成 graph3_cache.npz
- 直接在本脚本里从 5min 数据构建【图三：尾部时间段收益 -> 滚动20天相关系数 -> 01化二值图】
- 然后训练一个 RealGraph
- 训练集按季度输出
- 测试集按月份输出
...
说明：
- 除了“图构造”不同外，其余框架与前面 Graph1 / Graph2 修改版保持一致
- 图三默认使用 near_close（收盘前最后30分钟）
- 如需下午时段，请把 TAIL_MODE 改成 "afternoon"
"""
...
# ===== 图三：尾部时间段收益 =====
TAIL_MODE = "near_close"   # "near_close" or "afternoon"

AFTERNOON_START = "13:00"
AFTERNOON_END   = "15:00"
AFTERNOON_RET_PER_DAY = 24

CLOSE_END = "15:00"
NEAR_CLOSE_MINUTES = 30
NEAR_CLOSE_RET_PER_DAY = max(1, NEAR_CLOSE_MINUTES // 5)

SEG_RET_PER_DAY = AFTERNOON_RET_PER_DAY if TAIL_MODE == "afternoon" else NEAR_CLOSE_RET_PER_DAY

# ===== 图参数 =====
ROLL_W = 20
TOPK = 40

# 这里修正成 0：graph[t] 本身由 [t-20, ..., t-1] 构成，用来预测 y[t]
...
# ===== 输出目录 =====
_graph_tag = (
    f"afternoon_{AFTERNOON_START.replace(':','')}-{AFTERNOON_END.replace(':','')}"
    if TAIL_MODE == "afternoon"
    else f"near_close_last{NEAR_CLOSE_MINUTES}m"
)

OUT_DIR = rf"D:\PythonProject\LASSO_FINAL\graph\{EXP_NAME}_{_graph_tag}_realgraph_embedded"
...
GRAPH_CACHE = os.path.join(
    GRAPH_DIR,
    f"graph3_{_graph_tag}_roll{ROLL_W}_corr_BINARY01_abs{int(ABS_CORR)}_mkt{int(MARKET_REMOVE)}"
    f"_topk{TOPK}_M{SEG_RET_PER_DAY}_binTau{BIN_TAU}_full{int(REQUIRE_FULL_WINDOW)}_block{CPU_BLOCK}.npz"
)
...
# =======================
# 2) 5min 文件读取与 segment returns 构建
# =======================
...
def extract_segment_logret_from_day(df_day: pd.DataFrame) -> Optional[np.ndarray]:
    if df_day.empty:
        return None

    df_day = df_day.sort_values(TIME_COL).copy()
    t = pd.to_datetime(df_day[TIME_COL], errors="coerce")
    close = pd.to_numeric(df_day[CLOSE_COL], errors="coerce").astype(np.float32).to_numpy()

    ok = np.isfinite(close) & t.notna().to_numpy()
    if ok.sum() < 2:
        return None
    ...
    tmin = (t.dt.hour.to_numpy() * 60 + t.dt.minute.to_numpy()).astype(np.int32)

    if TAIL_MODE == "afternoon":
        start_min = hhmm_to_min(AFTERNOON_START)
        end_min   = hhmm_to_min(AFTERNOON_END)
    else:
        end_min   = hhmm_to_min(CLOSE_END)
        start_min = end_min - int(NEAR_CLOSE_MINUTES)

    mask = (tmin >= start_min) & (tmin <= end_min)
    close_seg = close[mask]

    need_closes = SEG_RET_PER_DAY + 1
    if close_seg.size < need_closes:
        return None

    close_seg = close_seg[-need_closes:]
    if np.any(~np.isfinite(close_seg)) or np.any(close_seg <= 0):
        return None

    ret = np.diff(np.log(close_seg + 1e-12)).astype(np.float32)
    if ret.size != SEG_RET_PER_DAY or (not np.all(np.isfinite(ret))):
        return None

    if CLIP_RET and CLIP_RET > 0:
        ret = np.clip(ret, -float(CLIP_RET), float(CLIP_RET)).astype(np.float32)
...
def build_or_load_returns_memmap(dates: List[pd.Timestamp], codes: List[str]):
    ...
    RET_MEMMAP_PATH = os.path.join(CACHE_DIR, f"segret_{_graph_tag}_T{T}_N{N}_M{M}.dat")
    RET_VALID_PATH  = os.path.join(CACHE_DIR, f"segret_{_graph_tag}_valid_T{T}_N{N}.npy")
    ...
    print(f"[INFO] 读取 5min 并构建 segment returns memmap | tag={_graph_tag} | M={M}")
    ...
    missing_files = 0
    for code in tqdm(codes, desc="5min->segret"):
        ...
        for d, sub in df.groupby("date", sort=False):
            ...
            r = extract_segment_logret_from_day(sub)
            if r is None:
                continue
    ...
    print(f"[INFO] segment returns 构建完成 | missing_files={missing_files} | 保存: {RET_MEMMAP_PATH}")
    return np.memmap(RET_MEMMAP_PATH, dtype="float32", mode="r", shape=(T, N, M)), valid


# =======================
# 3) Graph-3：CPU block 相关 TopK
# =======================
def corr_topk_cpu_block(X: np.ndarray, topk: int, block: int = 256) -> Tuple[np.ndarray, np.ndarray]:
    """
    X: (N,L) float32, row-wise standardized
    return idx:(N,topk) int64, val:(N,topk) float32
    """
    X = np.ascontiguousarray(X, dtype=np.float32)
    N, L = X.shape
    denom = np.float32(max(L - 1, 1))

    idx_out = np.empty((N, topk), dtype=np.int64)
    val_out = np.empty((N, topk), dtype=np.float32)
    ...
    for i0 in range(0, N, block):
        ...
        idx_out[i0:i1] = np.take_along_axis(part, order, axis=1).astype(np.int64)
        val_out[i0:i1] = np.take_along_axis(part_val, order, axis=1).astype(np.float32)
...
# 4) Graph-3：构建二值图
# =======================
def build_graph3_binary01(dates: List[pd.Timestamp], codes: List[str], ret_mmap: np.memmap, valid: np.ndarray):
    ...
    print("[INFO] 构图：Graph-3 tail segment -> rolling corr -> TopK -> 01 -> undirected -> noself")
    print(f"[INFO] tag={_graph_tag} T={T} N={N} window={ROLL_W} L={L} TOPK={topk_eff} "
          f"market_remove={MARKET_REMOVE} BIN_TAU={BIN_TAU} block={CPU_BLOCK} require_full={REQUIRE_FULL_WINDOW}")

    dates_np = pd.to_datetime(pd.Series(dates)).to_numpy(dtype="datetime64[ns]")

    for t in tqdm(range(T), desc=f"build graph3 ({_graph_tag})"):
        if t < ROLL_W:
            continue
        ...
        idx, val = corr_topk_cpu_block(X, topk_eff, block=CPU_BLOCK)
    ...
    meta = dict(
        graph_kind="g3",
        graph_tag=_graph_tag,
        tail_mode=TAIL_MODE,
        roll_window_days=int(ROLL_W),
        ret_per_day=int(SEG_RET_PER_DAY),
        topk=int(topk_eff),
        ...
        for t in range(loop_start, train_end_idx + 1):
            ...
            if (nbr_i is not None) and (LAPLACIAN_LAM > 0):
                if USE_NORMALIZED_LAPLACIAN:
                    loss = loss + LAPLACIAN_LAM * laplacian_loss_knn_normalized(pred, nbr_i, nbr_w_raw, deg_raw)
                else:
                    loss = loss + LAPLACIAN_LAM * laplacian_loss_knn_unnorm(pred, nbr_i, nbr_w_raw)
...
def main():
    ...
    print(f"[INFO] Graph-3 tag={_graph_tag} | RET_M={SEG_RET_PER_DAY}")
    ...
    g_dates, g_nbr_idx, g_nbr_w, g_node_valid = build_graph3_binary01(
    ...
    summary_df = pd.DataFrame([{
        "model": f"{EXP_NAME}_{_graph_tag}_RealGraph",
