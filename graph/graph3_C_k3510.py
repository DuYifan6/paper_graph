# -*- coding: utf-8 -*-
"""
Graph3 RealGraph | Near-close tail binary graph tuned C version for K=3/5/10
...
TAIL_RET = 24   # 使用收盘前最后24个5min收益（约最后两小时）
...
# ===== 输出目录 =====
OUT_DIR = rf"D:\PythonProject\LASSO_FINAL\graph_调整\graph3_tailbinary"
...
GRAPH_CACHE = os.path.join(
    GRAPH_DIR,
    f"roll{ROLL_W}_tail{TAIL_RET}_binary_abs{int(ABS_CORR)}_mkt{int(MARKET_REMOVE)}_topk{TOPK}_shift{GRAPH_SHIFT_TRADINGDAY}"
...
def build_rolling_corr_graph(dates: List[pd.Timestamp], codes: List[str], ret_mmap: np.memmap, valid: np.ndarray):
    """
    Graph3: near-close tail binary graph.
    - use only the last TAIL_RET intraday 5m log returns from each day
    - stack over a rolling ROLL_W-day window
    - compute abs correlation, Top-K, threshold, then binarize edges
    """
    ...
    print(f"[INFO] 构图：tail binary corr | use_gpu={use_gpu} device={device}")
    print(f"[INFO] T={T} N={N} window={ROLL_W} tail_ret={TAIL_RET} L={L} TOPK={topk_eff} market_remove={MARKET_REMOVE} require_full={REQUIRE_FULL_WINDOW}")

    dates_np = pd.to_datetime(pd.Series(dates)).to_numpy(dtype="datetime64[ns]")

    for t in tqdm(range(T), desc="build tail binary graphs"):
        if t < ROLL_W:
            continue

        win_ret = np.array(ret_mmap[t - ROLL_W:t, :, -TAIL_RET:], dtype=np.float32)
    ...
    meta = dict(
        graph_type="near_close_tail_binary_abs_corr_from_5m_logret",
        ...
        no_self_loop=True,
        note="Graph(day t) uses the last TAIL_RET 5m returns from window [t-ROLL_W, t-1]. Training uses graph(t-1) to predict y(t)."
    )
